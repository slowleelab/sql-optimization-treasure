...
```bash
# 默认在 MySQL 8.0 上运行
./scripts/run-case.sh 38-redundant-index-cleanup

# 在 MySQL 5.7 上运行（对比）
./scripts/run-case.sh 38-redundant-index-cleanup --ver 5.7

# 跳过造数据重跑
./scripts/run-case.sh 38-redundant-index-cleanup --no-seed
```
