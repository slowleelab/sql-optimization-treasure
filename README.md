...
> 每个案例都带百万级真实数据，Docker 一键复现，bad/good EXPLAIN 量化对比
...
| # | 案例 | 难度 | 版本 |
|---|------|------|------|
| 10 | 子查询改写为 JOIN | ⭐⭐ | 5.7 & 8.0 |
| 11 | COUNT(*) 慢查询优化 | ⭐⭐ | 5.7 & 8.0 |
| 12 | [GROUP BY filesort 优化](docs/cases/query-rewrite/12-group-by-filesort.md) | ⭐⭐ | 5.7 & 8.0 |
| 13 | 大 IN 列表优化 | ⭐⭐ | 5.7 & 8.0 |
| 14 | EXISTS vs IN | ⭐⭐ | 5.7 & 8.0 |

### 三、JOIN 优化（4 个）
| # | 案例 | 难度 | 版本 |
|---|------|------|------|
| 15 | [小表驱动大表](docs/cases/join/15-small-drive-large.md) | ⭐⭐ | 5.7 & 8.0 |
| 16 | 被驱动表无索引的灾难 | ⭐⭐ | 5.7 & 8.0 |
| 17 | Hash Join vs BNL | ⭐⭐⭐ | 8.0+ |
| 18 | 多表 JOIN 顺序控制 | ⭐⭐⭐ | 5.7 & 8.0 |

### 四、DDL 与大表（3 个）
| # | 案例 | 难度 | 版本 |
|---|------|------|------|
| 19 | 大表加索引 (Online DDL) | ⭐⭐⭐ | 5.7 & 8.0 |
| 20 | TEXT/BLOB 字段陷阱 | ⭐⭐ | 5.7 & 8.0 |
| 21 | 大表 DELETE 分批 | ⭐⭐ | 5.7 & 8.0 |

### 五、架构级优化（4 个）
| # | 案例 | 难度 | 版本 |
|---|------|------|------|
| 22 | 多条件动态筛选索引设计 | ⭐⭐⭐ | 5.7 & 8.0 |
| 23 | 报表统计汇总表 | ⭐⭐ | 5.7 & 8.0 |
| 24 | 冷热数据分离 | ⭐⭐⭐ | 5.7 & 8.0 |
| 25 | 秒杀场景库存扣减 | ⭐⭐⭐ | 5.7 & 8.0 |
...
我们特别欢迎以下方向的贡献：
- 真实生产中遇到的优化案例
- 8.0 新特性（CTE、窗口函数、Hash Join）的优化实践
